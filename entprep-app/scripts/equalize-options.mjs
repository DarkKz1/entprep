// Equalize answer option lengths across all question files
// Usage: OPENAI_API_KEY=xxx node scripts/equalize-options.mjs
...
// Call GPT for ONE question at a time — simple and reliable
...
async function processSubject(sid, questions) {
  ...
  if (flagged.length === 0) {
    console.log(`  ${sid}: 0 flagged — skip`);
    return null; // no changes
  }

  console.log(`  ${sid}: ${flagged.length} flagged, processing...`);
  const updated = questions.map(q => ({ ...q })); // shallow copy
  let fixed = 0;
  let errors = 0;

  const results = await processPool(flagged, async (item, i) => {
    try {
      const newO = await fixOneQuestion(item.q);
      if (newO) {
        updated[item.idx] = { ...item.q, o: newO };
        fixed++;
...
function writeSubjectFile(sid, info, questions) {
  ...
  // Extract header comments (all lines starting with //)
...
async function processReading() {
  ...
  for (let pi = 0; pi < updatedPassages.length; pi++) {
    ...
    const results = await processPool(flagged, async (item) => {
      try {
        const newO = await fixOneQuestion(item.q);
        if (newO) {
          passage.qs[item.idx] = { ...item.q, o: newO };
          totalFixed++;
        }
        return newO;
      } catch { return null; }
...
async function main() {
  ...
  console.log('\n=== Done! ===');
  console.log('Next: npm run build && re-seed Supabase && deploy');
}
